...
#                                                                              #
#                                                                              #
#          Establish Log location, SQL Connection Ref, and Vars               #
#                                                                              #
#                                                                              #
################################################################################
#>
...
#   If the user is currently backing up to OneDrive exit;                      #
#   If they are not take steps to Prep, Clear, and Configure                   #
#                                                                              #
################################################################################
#>
...
if ($NeedToContinue -eq $true) {
    ...
    function Set-ShellRegs() {
        ...
        $SHK = @(
            ...
            [pscustomobject]@{PropName = 'My Music' ; PropValue = "C:\Users\$env:USERNAME\Music"; PropPath = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders' }
            [pscustomobject]@{PropName = '{1B3EA5DC-B587-4786-B4EF-BD1DC332AEAE}' ; PropValue = "C:\Users\$env:USERNAME\AppData\Roaming\Microsoft\Windows\Libraries"; PropPath = 'HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Folders' }
        )
    ...
    function Copy-UsrData () {
        ...
        $Global:CSCRoot = [System.IO.Path]::Combine($csc, $ShareX_FQDN)

        if ((Test-Path $Global:CSCRoot) -eq $false) {
            $Global:CSCRoot = [System.IO.Path]::Combine($csc, $ShareX_SMPL)
        }
        ...
        $global:SrcDestLap = New-Object -TypeName psobject -Property @{
            'Desktop'   = '"$($Global:CSCRoot)\Desktop" "$($global:FinalDest)\Desktop"' 
            'Pictures'  = '"$($Global:CSCRoot)\My Documents\My Pictures" "$($global:FinalDest)\Documents\Pictures"'
            'Documents' = '"$($Global:CSCRoot)\My Documents" "$($global:FinalDest)\Documents"'
        }
