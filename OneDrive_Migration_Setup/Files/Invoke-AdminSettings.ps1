...
#                                                                              #
#                                                                              #
#                   Establish Log location and Vars                            #
#                                                                              #
#                                                                              #
################################################################################
#>
...
function Revoke-CSC () {
    ...
    $OutNull = [reflection.assembly]::loadwithpartialname('System.Windows.Forms') 
    $OutNull = [reflection.assembly]::loadwithpartialname('System.Drawing')
    ...
    Try {
        ...
        $OutNull = Start-Process $OwnProc -ArgumentList $OwnArgs -WindowStyle Hidden -Wait -ErrorAction Stop
    }
    catch {
        Continue
    }
    ...
    try {
        if ((Get-ChildItem $NewCSCDir).Length -gt 0) {
            $OutNull = New-Item -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\csc\Parameters' -Name 'FormatDatabase' -Value 1 -ItemType DWord -ea Stop
        }
    }
    catch {
        Continue
